...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      passcodeSet: !!(admin && admin.sitePasswordHash),
    });
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (passcode === "") {
      admin.sitePasswordHash = "";
    } else {
      if (passcode.length < 4) {
        return NextResponse.json({ error: "Passcode must be at least 4 characters long" }, { status: 400 });
      }
      const saltRounds = 12;
      admin.sitePasswordHash = await bcrypt.hash(passcode, saltRounds);
    }

    await admin.save();
    return NextResponse.json({ success: true, passcodeSet: !!admin.sitePasswordHash });
  } catch (error) {
